...
或打开`.vmx`文件，添加一行`firmware = "efi"`,

## 确保处于UEFI引导
```sh
ls /sys/firmware/efi/efivars
```
能输出文件名则处于UEFI引导，否则不在
```sh
cat //sys/firmware/efi/fw_platform_size
```
返回64则64位UEFI，返回32则32位UEFI
...
### 安装内核内驱动程序`In-kernel drivers`
编辑`/etc/mkinitcpio.conf`文件，在`MODULES`中添加`vmw_balloon` `vmw_pvscsi` `vmw_vmci` `vsock` `vmw_vsock_vmci_transport`
```sh
mkinitcpio -p linux
```
